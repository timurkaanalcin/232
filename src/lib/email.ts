...
export function passwordResetEmail(resetUrl: string): Pick<SendEmailInput, "subject" | "html" | "text"> {
  return {
    subject: "CanlıSite şifrenizi sıfırlayın",
    text: `CanlıSite şifrenizi sıfırlamak için bir istek aldık.\n\nBağlantı 30 dakika içinde sona erer:\n${resetUrl}\n\nBu isteği siz yapmadıysanız bu e-postayı yok sayabilirsiniz.`,
...
<h1 style="font-size:18px;margin:0 0 16px">Şifrenizi sıfırlayın</h1>
<p style="font-size:14px;line-height:1.6;color:#404040">CanlıSite şifrenizi sıfırlamak için bir istek aldık. Bu bağlantı 30&nbsp;dakika içinde sona erer.</p>
<p style="margin:24px 0"><a href="${resetUrl}" style="background:#0a0a0a;color:#ffffff;padding:10px 20px;border-radius:8px;text-decoration:none;font-size:14px">Şifreyi sıfırla</a></p>
<p style="font-size:12px;color:#737373">Bu isteği siz yapmadıysanız bu e-postayı yok sayabilirsiniz. Şifreniz değişmez.</p>
</div></body></html>`,
  };
}
