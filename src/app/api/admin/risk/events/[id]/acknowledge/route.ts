...
import { acknowledgeRiskEvent } from "@/services/risk-events";
...
export const POST = apiHandler(
  async (request: Request, { params }: { params: Promise<{ id: string }> }) => {
    ...
    if (!parsed.success) {
      throw badRequest(parsed.error.issues[0]?.message ?? "Invalid acknowledgement");
    }

    const { event, changed } = await acknowledgeRiskEvent(db, id, {
      actorId: actor.id,
    ...
    if (changed) {
      await logAudit(db, {
        ...
        action: AUDIT_ACTIONS.ADMIN_RISK_EVENT_ACKNOWLEDGED,
